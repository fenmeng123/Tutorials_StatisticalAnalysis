...
server <- function(input, output) {
  output$confounder_plot <- renderPlotly({
    ...
    X_noise = rnorm(n, sd = input$noise_level)
    Y_noise = rnorm(n, sd = input$noise_level)
    
    for (i in seq_along(true_effect_C_X)) {
      if (input$noise_level > 0) {
        X <- X_basis + true_effect_C_X[i] * C + X_noise
        Y <- input$effect_X_Y * X + input$effect_C_Y * C + Y_noise
      } else {
    ...
    p <- ggplot(plot_data, aes(x = True_Effect_C_to_X)) +
      ...
      labs(title = "Controlling Confounder (C) in Regression Model",
           x = "True Effect of C to X",
           y = "Estimated Beta (X -> Y)") +
...
server <- function(input, output) {
  ...
  output$collider_plot <- renderPlotly({
    ...
    Y_basis <- rnorm(n)
    X_noise <- rnorm(n, sd = input$noise_level)
    C_noise <- rnorm(n, sd = input$noise_level)
    Y_noise <- rnorm(n, sd = input$noise_level) 
    ...
    for (i in seq_along(true_effect_X_C)) {
      if (input$noise_level > 0) {
        X <- X_basis + X_noise
        Y <- Y_basis + input$effect_X_Y * X + Y_noise
        C <- true_effect_X_C[i] * X + input$effect_Y_C * Y + C_noise
      } else {
        X <- X_basis
    ...
    p <- ggplot(plot_data, aes(x = True_effect_X_C)) +
      ...
      geom_line(aes(y = Estimated_Effect_Without_C, color = "Without C"), linewidth = 1) +
      geom_line(aes(y = input$effect_X_Y, color = "Ground Truth"), linetype = "dashed", linewidth = 1) +
      labs(title = "Controlling Collider in Regression Model",
           x = "True Effect of X to C",
           y = "Estimated Beta (X -> Y)") +
      scale_color_manual(values = c("With C" = "blue", "Without C" = "red", "Ground Truth" = "black"),
...
server <- function(input, output) {
  ...
  output$mediator_plot <- renderPlotly({
    ...
    for (i in seq_along(true_effect_X_C)) {
      if (input$noise_level > 0) {
        X <- X_basis + X_noise
        C <- C_basis + true_effect_X_C[i] * X + C_noise
        Y <- input$effect_C_Y * C + input$effect_X_Y * X +  Y_noise
      } else {
        X <- X_basis
    ...
    p <- ggplot(plot_data, aes(x = True_effect_X_C)) +
      ...
      labs(title = "Controlling Mediator in Regression Model",
           x = "True Effect of X to C",
...
``` {r DAG justification for JAACAP paper, echo = F}
...
1. **Setting the Seed:** The process starts by setting a seed (in this case, `set.seed(123)`) to ensure that the generated data is reproducible.

2. **Defining Parameters:** The number of observations (`n`) and means for each variable are defined. In this example, there are 11 variables, and their means are all set to zero.

3. **Creating a Covariance Matrix:** A covariance matrix (`cov_matrix`) is created with dimensions 11x11. Initially, all elements are set to zero.

4. **Setting Diagonal Elements:** The diagonal elements of the covariance matrix are set to 1. These represent the variances of the variables.

5. **Specifying Covariances:** Specific covariances between variables are specified by modifying elements of the covariance matrix. For example, `cov_matrix[1, 2]` represents the covariance between the first variable (SMA) and the second variable (Developmental_Measures). These covariances are adjusted based on your requirements.

6. **Symmetrizing the Matrix:** To ensure the covariance matrix is symmetric (as required by covariance matrices), it is symmetrized by adding its transpose to itself while subtracting the diagonal.

7. **Generating Correlated Data:** Using the 'MASS' package's `mvrnorm` function, correlated data is generated. This function generates random multivariate data with the specified means and covariance matrix. The generated data has 11 variables corresponding to the defined means and covariances.

8. **Assigning Variable Names:** The column names of the generated data are assigned based on the variables, such as "SMA," "Developmental_Measures," "Age," and so on.

### Behind the Output Plot

The output plot generated by the provided R code snippet displays the results of including a new covariate in our linear regression model. Here's what happens behind the scenes:

1. **Covariate Selection:** The primary goal is to identify which covariates (independent variables) have the most influence on the dependent variable "Developmental_Measures" when regressed against the independent variable "SMA" (screen time).

2. **Covariate Selection Method:** The 'chest' package is used to perform linear regression. 

```{r Example about R package chest, include=T,echo=F}
...
# Perform linear regression for covariate selection
...
### Interactive Chest Plot

This interactiv plot allows you to explore covariate identification using the R package 'chest' with an interactive plot. You can dynamically adjust the sample size and customize the covariance matrix to see how it impacts the selection of covariates. Here's how to use it:

1. **Sample Size Slider:** Start by adjusting the "Sample Size" slider. You can change the number of observations in your dataset by dragging the slider left or right. This will determine the size of the dataset used for analysis.

2. **Covariance Matrix Input:** Below the sample size slider, you will see a matrix where you can input custom covariance values. This matrix represents the relationships between different variables in your dataset. You can manually change the values to create different covariance structures. Here's how:

   - **Entering Custom Covariance Values:** Click on any cell in the matrix and type a numeric value. This value represents the covariance between the corresponding variables. For example, you can specify the covariance between "SMA" and "Developmental_Measures" by changing the (1, 2) cell.

   - **Understanding the Diagonal:** The diagonal of the matrix represents the variances of each variable and is automatically set to 1. You don't need to modify these values.

   - **Symmetric Matrix:** Remember that covariance matrices should be symmetric. As you enter values, the app will ensure that the matrix remains symmetric.

3. **Update Data Button:** Once you've adjusted the sample size and covariance matrix values, click the "Update Data" button. This action triggers the recalculation of covariate selection based on the modified dataset.

4. **Interactive Plot:** After clicking "Update Data," you'll see an interactive plot displayed in the main panel. This plot shows the results of covariate selection using LASSO regression. You can interact with the plot in the following ways:

   - **Variable Importance:** The plot displays coefficients for different covariates on the x-axis. Positive coefficients indicate a positive relationship with the dependent variable "Developmental_Measures," while negative coefficients indicate a negative relationship. The magnitude of coefficients reflects the strength of the relationship.

   - **Dynamic Updates:** Feel free to further adjust the sample size or covariance matrix values and click "Update Data" again to observe how these changes impact covariate selection.

```{r Interactive chest plot, include=T,echo=F,warning=FALSE,message=FALSE}
library(shiny)
library(MASS)
library(chest)
library(shinyMatrix)
...
# Define UI
ui <- fluidPage(
  titlePanel("Covariate Identification Using R package chest"),
  sidebarLayout(
    sidebarPanel(
